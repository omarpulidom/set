...
import { bodyPartFilters, filterCatalogExercises, imageForExercise, type CatalogBodyPart, type CatalogExercise } from '@/features/exercises/catalog'
...
const ExerciseRow = memo(function ExerciseRow({
  ...
  return (
    ...
    >
      <Image source={imageForExercise(exercise.id)} className='h-14 w-14 rounded-2xl bg-surface-card' />
      <View className='ml-4 flex-1'>
        <Text numberOfLines={1} className='font-geist-mono-semibold text-sm text-surface-dark'>{exercise.name}</Text>
        <Text numberOfLines={1} className='mt-1 font-geist-mono text-[10px] text-ink-muted'>{exercise.target} · {exercise.equipment}</Text>
      </View>
      {selected ? <Feather name='check' size={17} color={Colors.ink.soft} /> : <Feather name='plus' size={18} color={Colors.surface.dark} />}
    </TouchableOpacity>
  )
})
...
export default function ExercisePickerScreen() {
  ...
  const selectedIds = useMemo(() => new Set(exercises.map((exercise) => exercise.catalogExerciseId)), [exercises])
  const results = useMemo(() => filterCatalogExercises(deferredQuery, filter), [deferredQuery, filter])

  const openExercise = useCallback((exercise: CatalogExercise) => {
    if (selectedIds.has(exercise.id)) return
    router.push({ pathname: '/routine/exercises/[exerciseId]', params: { exerciseId: exercise.id } })
  }, [router, selectedIds])
  ...
  return (
    <SafeAreaView className='flex-1 bg-surface' edges={['top', 'left', 'right']}>
      <View className='px-5 pt-5'>
        ...
        <Text className='mt-8 font-geist-mono-semibold text-3xl tracking-[-1px] text-surface-dark'>Ejercicios</Text>
      ...
      <View className='mt-4'>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20 }}>
          {bodyPartFilters.map((item) => (
            ...
            >
              <Text className={`font-geist-mono text-xs ${filter === item.id ? 'text-surface-card' : 'text-ink-muted'}`}>{item.label}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      </View>
      ...
      <FlatList
        ...
        getItemLayout={(_, index) => ({ length: EXERCISE_ROW_HEIGHT, offset: EXERCISE_ROW_HEIGHT * index, index })}
