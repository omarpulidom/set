...
const reactionLabels: Record<TicketReaction, string> = { fire: '🔥', clap: '👏', strong: '💪' }
...
export function TicketFrame({
  ...
  return (
    <TouchableOpacity activeOpacity={onPress ? 0.86 : 1} onPress={onPress} className='rounded-3xl bg-surface-muted p-5'>
      ...
      <View className='mt-4 flex-row items-start justify-between'>
        <View>
          <Text className='font-geist-mono-semibold text-2xl tracking-[-1px] text-surface-dark'>{ticket.routineName}</Text>
        </View>
        <TouchableOpacity onPress={onShare} className='h-8 w-8 items-center justify-center rounded-full bg-surface-soft'>
      ...
      {social ? (
        <>
          <Text className='mt-3 font-geist-mono text-[10px] text-ink-muted'>{ticket.circles.join(' · ')}</Text>
          <View className='mt-4 flex-row border-t border-border-soft pt-3'>
            {(Object.keys(reactionLabels) as TicketReaction[]).map((reaction) => (
              ...
              >
                ...
                <Text className={`ml-1 font-geist-mono text-[10px] ${ticket.myReaction === reaction ? 'text-surface-card' : 'text-ink-muted'}`}>
