...
half4 main(float2 xy) {
  ...
  // Controls how quickly cross arms grow
  float shapedInk =
    pow(
      max(ink, 0.00001),
      max(gamma, 0.00001)
  ...
  // Cross half-thickness — at full ink the arms
  // reach the cell edges (radius = cellSize/2
  // covers every point where min(|x|,|y|) is small).
  float radius =
    shapedInk * cellSize * 0.5;
  ...
  //   * * *
  //
  // min(|x|, |y|) is small on the arms (horizontal
  // and vertical) and large in the corners, so
  // increasing radius "fattens" the cross.
  float d =
    min(
      abs(local.x),
      abs(local.y)
...
export type CrossHalftoneUniforms = {
  cellSize: number
  contrast: number
  brightness: number
  threshold: number
